(function() {
	...
	$(function() {
		...
		$("#cardNameForm").submit(function(e) {
			e.preventDefault();
			...
			if (!/^[A-Za-z0-9'\-]+( [A-Za-z0-9'\-]+)*$/.test(name)) {
				alert("Card Name must contain only English letters, numbers, spaces, hyphen, and apostrophe");
				return;
			}
			...
			$.ajax({
				url : APP_CTX + "/master/card/saveCardName",
				type : "POST",
				data : fd,
				processData : false,
				contentType : false,
				success : function(res) {
					if (res === "SUCCESS") {
						window.location.href = APP_CTX + "/master/card/viewCardName";
					} else {
						alert(res);
					}
				},
